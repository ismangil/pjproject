FROM ubuntu:14.04
RUN apt-get update && apt-get install -y build-essential subversion python-svn curl libasound2-dev
...
COPY . /pjproject
RUN cd /pjproject && timestamp=$(date +%Y%m%d-%H%M) && python ~/lib/python/ccdash.py configure "./configure" \
